...
class BarGenerator:
    ...
    def update_tick(self, tick: TickData) -> None:
        ...
        self.last_tick = tick
    def update_v_tick(self, tick: TickData) -> None:
        """
        Update new tick data into generator.
        """
        ...
        if not self.bar:
            ...
            (self.bar.volume>=self.volumebar)
        ):
            self.on_bar(self.bar)
    ...
    def update_bar(self, bar: BarData) -> None:
        ...
        elif self.interval == Interval.VOLUME:
            self.update_bar_volume(bar)
        else:
    ...
    def update_bar_volume(self, bar: BarData) -> None:
        """"""
        # If not inited, create window bar object
        if not self.window_bar:
        ...
        # Check if window bar completed
        if self.window_bar.volume + 1>self.window*self.volumebar:
            self.on_window_bar(self.window_bar)
            self.window_bar = None
