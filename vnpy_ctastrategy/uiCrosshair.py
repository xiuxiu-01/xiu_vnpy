...
class Crosshair(QtCore.QObject):
    ...
    def show_kline_info(self, x_axis: int, y_axis: float):
        ...
        open_text_color = self.get_color(kline.open_price, pre_kline.close_price)
        high_text_color = self.get_color(kline.high_price, pre_kline.close_price)
        low_text_color = self.get_color(kline.low_price, pre_kline.close_price)
        close_text_color = self.get_color(kline.close_price, pre_kline.close_price)
        ...
        self.__text_info.setHtml(
            f'<div style="background-color:#3F424D; font-size: 16px; color: white"> \
                ...
                <span style="color: {open_text_color};">{kline.open_price}</span><br> \
                <span style="color: {high_text_color};">最高价</span><br> \
                <span style="color: {high_text_color};">{kline.high_price}</span><br> \
                <span style="color: {low_text_color};">最低价</span><br> \
                <span style="color: {low_text_color};">{kline.low_price}</span><br> \
                <span style="color: {close_text_color};">收盘价</span><br> \
                <span style="color: {close_text_color};">{kline.close_price}</span><br> \
                <span>成交量</span><br> \
