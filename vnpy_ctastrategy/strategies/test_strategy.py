...
class RBreakStrategy(CtaTemplate):
    ...
    def on_stop_order(self, stop_order: StopOrder):
        ...
        """
        pass
class breakopenStrategy(CtaTemplate):
    """"""
    ...
    short_stop = 0
    long_stop = 0
    enter_time = time(hour=21, minute=30)
    exit_time = time(hour=22, minute=51)
    ...
    def on_bar(self, bar: BarData):
        ...
        if bar.datetime.time()<time(hour=9, minute=self.donchian_window):
            self.tend_high, self.tend_low = am.donchian(self.donchian_window)
            return
        if bar.datetime.time() < self.enter_time:
            if self.pos == 0:
                if bar.close_price>self.tend_high and bar.close_price>self.buy_setup:
                    self.buy(bar.close_price* 1.01,self.fixed_size,net=True)
                if bar.close_price<self.tend_low and bar.close_price<self.sell_setup:
                    self.short(bar.close_price* 0.99,self.fixed_size,net=True)
                
            elif self.pos > 0 and ((bar.close_price<self.tend_low or bar.close_price<self.sell_setup)):#bar.close_price<self.intra_trade_high * (1 - self.trailing_long / 100) or 
                #self.tend_high,self.tend_low=am.donchian(self.donchian_window)
                self.tend_high,self.tend_low=10000,0
                self.sell(bar.close_price* 0.99, abs(self.pos))
                # self.short(bar.close_price,self.fixed_size,net=True)

            elif self.pos < 0 and ((bar.close_price>self.tend_high or bar.close_price>self.buy_setup)):#bar.close_price>self.intra_trade_low * (1 + self.trailing_short / 100) or 
                #self.tend_high,self.tend_low=am.donchian(self.donchian_window)
                self.tend_high,self.tend_low=10000,0
                self.cover(bar.close_price* 1.01, abs(self.pos))
                # self.buy(bar.close_price,self.fixed_size,net=True)
            ...
            if self.pos > 0 and (((bar.close_price<self.tend_low or bar.close_price<self.sell_setup)) ):#bar.close_price<self.intra_trade_high * (1 - self.trailing_long / 100) or 
                #self.tend_high,self.tend_low=am.donchian(self.donchian_window)
                self.tend_high,self.tend_low=10000,0
                self.sell(bar.close_price* 0.99, abs(self.pos))
                # self.short(bar.close_price,self.fixed_size,net=True)
...
from datetime import time
class breakopen2Strategy(CtaTemplate):
    """"""
    ...
    def on_bar(self, bar: BarData):
        ...
        if bar.datetime.time() < self.enter_time:
            if self.pos == 0:
                ...
                self.tend_high,self.tend_low=10000,0
                self.cover(bar.close_price* 1.01, abs(self.pos))
            elif self.pos > 0 and ((bar.close_price-bar.open_price<-15)):
                self.tend_high,self.tend_low=10000,0
                self.sell(bar.close_price* 0.99, abs(self.pos))
            elif self.pos < 0 and ((bar.close_price-bar.open_price>15)):
                self.tend_high,self.tend_low=10000,0
                self.cover(bar.close_price* 1.01, abs(self.pos))
                # self.buy(bar.close_price,self.fixed_size,net=True)
            # if self.pos > 0 and bar.close_price<self.tend_high and bar.close_price<self.buy_setup and (bar.close_price<self.tend_low or bar.close_price<self.sell_setup):
            #     self.short(bar.close_price,self.fixed_size,net=True)
            # elif self.pos < 0 and bar.close_price>self.tend_low and bar.close_price>self.sell_setup and (bar.close_price>self.tend_high or bar.close_price>self.buy_setup):
            #     self.buy(bar.close_price,self.fixed_size,net=True)
